...
import { Download, FileText, Calendar, Check, X, Filter } from 'lucide-react';
import { AreaChart, Area, XAxis, YAxis, Tooltip, ResponsiveContainer } from 'recharts';
import { jsPDF } from 'jspdf';
import { AppIconFull } from '../components/AppIcon';
import { exportFile } from '../utils/nativeFileExport';
import { fitTextToWidth } from '../utils/reportGenerator';
...
export const Reports: React.FC = () => {
  ...
  // Filter transactions by selected account (All accounts or specific account)
  const accountFilteredTxs = transactions.filter((tx) => {
    if (selectedAccountId === 'all') return true;
    return tx.accountId === selectedAccountId || (tx.type === 'transfer' && tx.toAccountId === selectedAccountId);
  });

  const exportFilteredTxs = accountFilteredTxs.filter((tx) => {
    return tx.date >= exportStartDate && tx.date <= exportEndDate;
  });

  // Trigger loading skeleton on period or account change
  useEffect(() => {
    setLoading(true);
  ...
  // Close export modal when mobile back button is pressed
  useEffect(() => {
  ...
  // 2. Filter transactions by period from the account-filtered dataset
  const filteredTxs = accountFilteredTxs.filter((tx) => {
  ...
  const handleExportPDF = async () => {
    ...
    // Use 'Rs.' as a fallback for '₹' since jsPDF Helvetica doesn't support the Indian Rupee symbol out of the box
    const safeCurrency = currency === '₹' ? 'Rs. ' : currency;

    const addPageHeaderAndFooter = (pageNumber: number) => {
      // Header Background
      doc.setFillColor(11, 18, 32); // #0B1220 dark navy
      doc.rect(0, 0, 210, 35, 'F');

      // Header Text
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(24);
      doc.setTextColor(16, 185, 129); // #10B981 accent green
      doc.text('Pocket-Ledger.pro', 15, 20);

      doc.setFont('helvetica', 'normal');
      doc.setFontSize(10);
      doc.setTextColor(200, 200, 200);
      doc.text('Custom Financial Summary Report', 15, 27);
      
      // Right side text
      doc.setFontSize(9);
      doc.text(`Generated: ${new Date().toLocaleDateString()}`, 195, 20, { align: 'right' });
      doc.text(`Page ${pageNumber}`, 195, 27, { align: 'right' });

      // Footer
      doc.setFillColor(248, 250, 252); // light slate for footer
      doc.rect(0, 284, 210, 13, 'F');
      doc.setFontSize(7.5);
      doc.setTextColor(150, 150, 150);
      doc.text('Generated by Pocket-Ledger.pro — Private Offline Personal Finance', 105, 290, { align: 'center' });
    ...
    doc.text(`Date Range: ${exportStartDate} to ${exportEndDate}`, 15, 45);
    ...
    doc.rect(15, 50, 56, 22, 'FD');
    doc.setFontSize(9);
    doc.setTextColor(100, 116, 139);
    doc.text('Total Inflows', 43, 57, { align: 'center' });
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(16, 185, 129);
    doc.text(`${safeCurrency}${totalIn.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, 43, 65, { align: 'center' });

    // Outflow Box
    doc.setFillColor(254, 242, 242); // red-50
    doc.setDrawColor(239, 68, 68); // red-500
    doc.rect(77, 50, 56, 22, 'FD');
    doc.setFontSize(9);
    doc.setTextColor(100, 116, 139);
    doc.setFont('helvetica', 'normal');
    doc.text('Total Outflows', 105, 57, { align: 'center' });
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(239, 68, 68);
    doc.text(`${safeCurrency}${totalOut.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, 105, 65, { align: 'center' });

    // Net Balance Box
    doc.setFillColor(248, 250, 252); // slate-50
    doc.setDrawColor(148, 163, 184); // slate-400
    doc.rect(139, 50, 56, 22, 'FD');
    doc.setFontSize(9);
    doc.setTextColor(100, 116, 139);
    doc.setFont('helvetica', 'normal');
    doc.text('Net Balance', 167, 57, { align: 'center' });
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(netFlow >= 0 ? 16 : 239, netFlow >= 0 ? 185 : 68, netFlow >= 0 ? 129 : 68);
    doc.text(`${safeCurrency}${netFlow.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, 167, 65, { align: 'center' });

    // Table Header
    let y = 85;
    doc.setFillColor(11, 18, 32); // Dark bg for table header
    ...
    exportFilteredTxs.slice(0, 50).forEach((tx) => {
      if (y > 275) {
        pageNum++;
        doc.addPage();
        addPageHeaderAndFooter(pageNum);
        y = 50; // Start lower on subsequent pages
      ...
      cols.forEach(col => {
        ...
        if (col.id === 'date') text = tx.date;
          ...
          // Optional: Color code type column
          if (tx.type === 'income') doc.setTextColor(16, 185, 129);
          else if (tx.type === 'expense') doc.setTextColor(239, 68, 68);
          else doc.setTextColor(99, 102, 241); // indigo for transfer
        }
        else if (col.id === 'amount') text = `${safeCurrency}${tx.amount.toLocaleString('en-IN')}`;
        else if (col.id === 'notes') text = tx.notes || '';

        const safeCellText = fitTextToWidth(doc, text, col.width - 4);
        doc.text(safeCellText, col.x + 2, y);
        doc.setTextColor(30, 41, 59); // reset color for next columns
      });
  ...
  return (
    <div className="pb-24 pt-6 px-4 max-w-lg mx-auto space-y-6 transition-all duration-300">
      ...
      {/* Account Filter Bar: All Accounts | Select Account */}
      <div className="space-y-1.5 text-left">
        <div className="flex items-center justify-between px-1">
          <div className="flex items-center space-x-1.5">
            <Filter className="w-3 h-3 text-text-subtle" />
            <span className="text-[11px] font-bold text-text-subtle uppercase tracking-wider font-body">
              Account Filter
            </span>
          </div>
          {selectedAccountId !== 'all' && (
            <button
              ...
              className="text-[10px] text-accent-green hover:underline cursor-pointer font-semibold"
            >
              Reset to All Accounts
            </button>
          )}
        </div>
        <div className="flex space-x-2 overflow-x-auto pb-1 scrollbar-none no-scrollbar">
          <button
            id="reports-filter-all"
            type="button"
            onClick={() => setSelectedAccountId('all')}
            className={`px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition cursor-pointer flex items-center space-x-1.5 min-h-[36px] ${
              selectedAccountId === 'all'
                ? 'bg-accent-green text-bg-base shadow-xs'
                : 'bg-bg-surface border border-border-custom text-text-secondary hover:text-text-primary'
            }`}
          >
            <span>All Accounts</span>
            <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-bold ${selectedAccountId === 'all' ? 'bg-black/20 text-bg-base' : 'bg-white/10 text-text-subtle'}`}>
              {accounts.length}
            </span>
          </button>
          {accounts.map(acc => (
            ...
            >
              <span>{acc.name}</span>
              <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-bold ${selectedAccountId === acc.id ? 'bg-black/20 text-bg-base' : 'bg-white/10 text-text-subtle'}`}>
                {acc.type}
              </span>
            </button>
          ))}
